...
func init() {
	Assets = rice.MustFindBox("res").HTTPBox()
	...
	//http.Handle("/res/", http.FileServer(Assets))
}
...
func (s *Supervisor) renderHTML(r *http.Request, w http.ResponseWriter, name string, data interface{}) {
	...
	if data == nil {
		...
		if err == nil {
			wc.User = user.Username + "@" + cfg.Server.ServerName
		}
		data = wc
	}
...
func (s *Supervisor) hIndex(w http.ResponseWriter, r *http.Request) {
	s.renderHTML(r, w, "index", nil)
}

func (s *Supervisor) hSetting(w http.ResponseWriter, r *http.Request) {
	name := mux.Vars(r)["name"]
	s.renderHTML(r, w, "setting", map[string]string{
		"Name": name,
	})
}
